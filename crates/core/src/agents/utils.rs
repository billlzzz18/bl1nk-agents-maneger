use crate::agents::types::{
    AgentPromptMetadata, AgentCategory, AgentCost, DelegationTrigger, BuiltinAgentName,
    AgentConfig, AgentName, AgentOverrideConfig, AgentOverrides
};
use std::path::Path;
...
use crate::agents::prompt_builder::{
    AvailableAgent, AvailableCategory, AvailableSkill
};
use std::collections::HashMap;
use serde::{Deserialize, Serialize};

type AgentFactory = fn(model: &str) -> AgentConfig;

const DEFAULT_CATEGORIES: &str = "default_categories"; // Placeholder for actual default categories
const CATEGORY_DESCRIPTIONS: &str = "category_descriptions"; // Placeholder for actual category descriptions

// Re-using AvailableAgent from prompt_builder

// Agent sources mapping
fn get_agent_sources() -> HashMap<BuiltinAgentName, Box<dyn Fn(&str) -> AgentConfig>> {
    let mut sources = HashMap::new();
    
    sources.insert(BuiltinAgentName::Orchestrator, Box::new(|model: &str| create_orchestrator_agent(model)));
    sources.insert(BuiltinAgentName::Expert, Box::new(|model: &str| create_expert_agent(model)));
    sources.insert(BuiltinAgentName::Researcher, Box::new(|model: &str| create_researcher_agent(model)));
    sources.insert(BuiltinAgentName::Explorer, Box::new(|model: &str| create_explorer_agent(model)));
    sources.insert(BuiltinAgentName::Observer, Box::new(|model: &str| create_observer_agent(model)));
    sources.insert(BuiltinAgentName::Consultant, Box::new(|model: &str| create_consultant_agent(model)));
    sources.insert(BuiltinAgentName::Auditor, Box::new(|model: &str| create_auditor_agent(model)));
    // Note: Atlas is handled specially in create_builtin_agents()
    // because it needs OrchestratorContext, not just a model string
    sources.insert(BuiltinAgentName::Manager, Box::new(|_model: &str| {
    ...
    sources.insert(BuiltinAgentName::Planner, Box::new(|model: &str| create_planner_agent(model, None, None, None)));
    sources.insert(BuiltinAgentName::OrchestratorJunior, Box::new(|model: &str| {
...
fn get_agent_metadata() -> HashMap<BuiltinAgentName, AgentPromptMetadata> {
    ...
    metadata.insert(BuiltinAgentName::Researcher, RESEARCHER_PROMPT_METADATA.clone());
    ...
    metadata.insert(BuiltinAgentName::Consultant, CONSULTANT_PROMPT_METADATA.clone());
    ...
    metadata.insert(BuiltinAgentName::OrchestratorJunior, AgentPromptMetadata {
        ...
        use_when: Some(vec!["Task is atomic".to_string(), "No delegation needed".to_string()]),
...
pub fn build_agent(
    source: Box<dyn Fn(&str) -> AgentConfig>,
    model: &str,
    categories: Option<&CategoriesConfig>,
    git_master_config: Option<&GitMasterConfig>,
) -> AgentConfig {
    let mut base = source(model);
    
    // Apply category configuration if available
    if let Some(cat_config) = categories {
        // Apply category-specific settings to the agent
        // This is a simplified version - actual implementation would depend on the structure of CategoriesConfig
        if let Some(ref mut base_model) = base.model {
            if base_model.is_empty() {
                // Apply category model if base model is empty
            }
        }
        
        // Apply other category-specific settings like temperature, etc.
        if base.temperature.is_none() {
            // Apply category temperature if not set
        }
    }
    ...
    if let Some(ref skills) = base.skills {
        if !skills.is_empty() {
            // Process skills and potentially modify the agent's prompt
            // This would involve resolving multiple skills similar to the TypeScript version
        }
    }
...
pub fn create_env_context() -> String {
    ...
    let locale = "en-US"; // Simplified - in real implementation, this would be configurable
...
fn merge_agent_config(
    base: &AgentConfig,
    override_config: &AgentOverrideConfig,
) -> AgentConfig {
    // This is a simplified implementation - a full implementation would need to properly merge
    // all fields from both structs
    let mut merged = base.clone();
    
    // Apply overrides from the override_config
...
pub async fn create_builtin_agents(
    ...
    categories: Option<&CategoriesConfig>,
    git_master_config: Option<&GitMasterConfig>,
) -> Result<HashMap<String, AgentConfig>, Box<dyn std::error::Error>> {
    let mut result = HashMap::new();
    let mut available_agents = Vec::new();
    
    // 1. Discover External Agents from Root (for Orchestrator prompt inclusion)
                ...
                if let Ok(json) = serde_json::from_str::<serde_json::Value>(&content) {
                    if let Some(agents_list) = json.get("agents").and_then(|a| a.as_array()) {
                        for agent_val in agents_list {
                            let id = agent_val.get("id").and_then(|v| v.as_str()).unwrap_or_default();
                            let name = agent_val.get("name").and_then(|v| v.as_str()).unwrap_or(id);
                            let desc = agent_val.get("description").and_then(|v| v.as_str()).unwrap_or_default();
                            let cat_str = agent_val.get("category").and_then(|v| v.as_str()).unwrap_or("utility");
                            ...
                            // Add to available agents for prompt inclusion
    ...
    // Process each agent source
    for (agent_name, source_fn) in agent_sources.iter() {
        let name_str = agent_name.to_string();
        
        // Skip certain agents or if disabled
        if name_str == "orchestrator" || name_str == "manager" || 
           disabled_agents.contains(&name_str) {
            continue;
        }
        
        // Get override for this agent
        ...
        let mut config = build_agent(
            Box::new(|m| source_fn(m)),
            &model,
            categories,
            git_master_config,
        );
        
        // Apply directory-specific context if needed (for librarian)
        // Apply directory-specific context if needed (for researcher)
        ...
        // Add to available agents if metadata exists
    ...
    if !disabled_agents.contains(&"manager".to_string()) {
        ...
        let manager_context = crate::agents::manager::ManagerContext {
            ...
            available_skills: vec![], // We'll need to pass actual skills if available
            user_categories: categories.cloned(), // Convert CategoriesConfig to HashMap if needed
        };
