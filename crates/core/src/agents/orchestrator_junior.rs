use crate::agents::types::{AgentConfig, is_gpt_model, AgentOverrideConfig};
...
pub fn create_orchestrator_junior_agent_with_overrides(
    ...
    let prompt_append = override_config.and_then(|o| o.prompt_append.as_ref()).map(|s| s.as_str());
    ...
    let base = AgentConfig {
        description: Some(override_config
            .and_then(|o| o.partial_agent_config.description.as_ref())
            .cloned()
            .unwrap_or_else(|| "Orchestrator-Junior - Focused task executor. Same discipline, no delegation.".to_string())),
        mode: Some("subagent".to_string()),
        model: Some(model),
        temperature: Some(temperature),
        max_tokens: Some(64000),
        prompt: Some(prompt),
        color: Some(override_config
