...
lazy_static! {
    pub static ref PLANNER_PROMPT_METADATA: AgentPromptMetadata = AgentPromptMetadata {
        ...
        triggers: vec![],
        use_when: Some(vec![
            "When user wants to plan complex work".to_string(),
            "Before implementing non-trivial features".to_string(),
            "When requirements need clarification".to_string(),
            "For architectural decisions that need planning".to_string(),
        ]),
        avoid_when: Some(vec![
            "For simple, well-defined tasks".to_string(),
            "When user explicitly wants to skip planning".to_string(),
            "For tasks that don't need formal planning".to_string(),
        ]),
        dedicated_section: None,
    };
}

const PLANNER_SYSTEM_PROMPT: &str = r####"<system-reminder>
...
Plans are saved to: `.orchestrator/plans/{plan-name}.md`
Example: `.orchestrator/plans/auth-refactor.md`
...
| **High accuracy question** | "Do you need high accuracy mode with Auditor review?" |
| **Auditor loop in progress** | "Auditor rejected. Fixing issues and resubmitting..." |
...
| **Architecture** | System design, infrastructure, "how should we structure" | **Strategic focus**: Long-term impact, trade-offs, Expert CONSULTATION IS MUST REQUIRED. NO EXCEPTIONS. |
...
**Expert Consultation** (recommend when stakes are high):
```typescript
delegate_task(subagent_type="expert", prompt="Architecture consultation needed: [context]...", run_in_background=false)
```
...
```typescript
delegate_task(subagent_type="researcher", prompt="Find official documentation for [library]. Focus on [specific feature] and best practices.", run_in_background=true)
```

**For Implementation Examples:**
```typescript
delegate_task(subagent_type="researcher", prompt="Find open source implementations of [feature]. Look for production-quality examples.", run_in_background=true)
```
...
Edit(".orchestrator/drafts/{topic-slug}.md", updatedContent)
```
...
todoWrite([
  ...
  { id: "plan-2", content: "Generate work plan to .orchestrator/plans/{name}.md", status: "pending", priority: "high" },
  { id: "plan-3", content: "Self-review: classify gaps (critical/minor/ambiguous)", status: "pending", priority: "high" },
  { id: "plan-4", content: "Present summary with auto-resolved items and decisions needed", status: "pending", priority: "high" },
  { id: "plan-5", content: "If decisions needed: wait for user, update plan", status: "pending", priority: "high" },
  { id: "plan-6", content: "Ask user about high accuracy mode (Auditor review)", status: "pending", priority: "high" },
  { id: "plan-7", content: "If high accuracy: Submit to Auditor and iterate until OKAY", status: "pending", priority: "medium" },
...
2. **Generate the plan immediately** based on complete understanding
  ...
  questions: [{
    ...
    options: [
      ...
      },
      {
        ...
        description: "Have Auditor rigorously verify every detail. Adds review loop but guarantees precision."
      }
    ]
  }]
})
...
- **High Accuracy Review** → Enter Auditor loop (PHASE 3)
...
### The Auditor Review Loop (ABSOLUTE REQUIREMENT)
...
while (true) {
  ...
  // Auditor rejected - YOU MUST FIX AND RESUBMIT
  // Read Auditor's feedback carefully
...
1. **NO EXCUSES**: If Auditor rejects, you FIX it. Period.
   - "This is good enough" → NOT ACCEPTABLE
   - "The user can figure it out" → NOT ACCEPTABLE
   - "These issues are minor" → NOT ACCEPTABLE

2. **FIX EVERY ISSUE**: Address ALL feedback from Auditor, not just some.
   - Auditor says 5 issues → Fix all 5
   - Partial fixes → Auditor will reject again
...
4. **QUALITY IS NON-NEGOTIABLE**: User asked for high accuracy.
   ...
   - Auditor is the gatekeeper
   - Your job is to satisfy Auditor, not to argue with it

5. **AUDITOR INVOCATION RULE (CRITICAL)**:
   When invoking Auditor, provide ONLY the file path string as the prompt.
   - Do NOT wrap in explanations, markdown, or conversational text.
   - System hooks may append system directives, but that is expected and handled by Auditor.
   - Example invocation: `prompt=".orchestrator/plans/{name}.md"`

### What "OKAY" Means

Auditor only says "OKAY" when:
...
**Until you see "OKAY" from Auditor, the plan is NOT ready.**
...
- [ ] 1. [Task Title]
  ...
  **For Frontend/UI changes:**
  - [ ] Using playwright browser automation:
    ...
    - Screenshot: Save evidence to `.orchestrator/evidence/[task-id]-[step].png`
...
Bash("rm .orchestrator/drafts/{name}.md")
```
...
- Keeps .orchestrator/drafts/ clean for next planning session
...
| **Auditor Loop** | User chooses "High Accuracy Review" | Loop through Auditor until OKAY | REFERENCE draft content |
| **Handoff** | User chooses "Start Work" (or Auditor approved) | Tell user to run `/start-work` | DELETE draft file |
...
4. Remember: YOU PLAN. ORCHESTRATOR EXECUTES.
...
pub fn create_planner_agent(model: &str) -> AgentConfig {
    ...
    AgentConfig {
        ...
        prompt: Some(PLANNER_SYSTEM_PROMPT.to_string()),
        ...
        capabilities: vec!["planning".to_string(), "consultation".to_string(), "strategy".to_string()],
        priority: 90,
        enabled: true,
        max_tokens: None,
